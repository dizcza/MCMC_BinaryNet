...
class DataSubset(torch.utils.data.TensorDataset):
    ...
    def __init__(self, dataset_cls, labels_keep: Tuple, train: bool, resize_to=(5, 5)):
        self.labels_keep = labels_keep
        self.train = train
        self.transform = transforms.Compose([transforms.Resize(size=resize_to), transforms.ToTensor()])
    ...
    def process_dataset(self, dataset: torch.utils.data.Dataset):
        ...
        data = (data - data_mean) / (data_std + 1e-7)
...
class MNIST56(DataSubset):
    """
    MNIST 5 and 6 digits, resized to 5 x 5.
    """
...
class FashionMNIST56(DataSubset):
    """
    FashionMNIST 5 and 6 classes, resized to 5 x 5.
    """
    def __init__(self, train=True):
...
class CIFAR10_56(DataSubset):
    """
    CIFAR10 5 and 6 classes, resized to 5 x 5.
    """
    def __init__(self, train=True):
